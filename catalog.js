...
function renderProducts(list) {
    ...
    list.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <p class="price">${product.price.toLocaleString()} грн</p>

            <button class="add-btn">Додати в кошик</button>
        `;
        // toast + (тут можна потім підключити кошик)
...
img.addEventListener("click", (e) => {
    e.stopPropagation(); // щоб не спрацьовувало інше
    openModal(product);
});
...
function openModal(product) {
    ...
    for (let key in product.specs) {
        specsHTML += `
            <p><strong>${key}:</strong> ${product.specs[key]}</p>
        `;
    }
    ...
    product.images.forEach(img => {
        imagesHTML += `<img src="${img}" class="thumb">`;
    });
    ...
    <div class="modal-content">
        ...
        <div class="modal-grid">
            ...
            <div class="modal-left">
                <img src="${product.img}" class="main-img">
...
`;
    ...
    // зміна головної картинки
    modal.querySelectorAll(".thumb").forEach(img => {
...
function showToast(message) {
    ...
    // Якщо контейнера немає в HTML, створюємо його самі
    if (!container) {
    ...
    // Анімація зникнення через 2.5 секунди
    setTimeout(() => {
        toast.classList.remove("show");
        setTimeout(() => toast.remove(), 300); // видаляємо з пам'яті після анімації
    }, 2500);
}
// ===== ДОДАВАННЯ В КОШИК =====
